...
export default async function handler(req, res) {
  ...
  try {
    ...
    ctx.fillText('THE ZENJAKU EXPERIMENT', 40, 70)

    ctx.fillStyle = '#888888'
    ctx.font = '14px monospace'
    ctx.fillText(`EXPERIMENT LOG · DAY ${day}`, 40, 95)

    // Stats line - more prominent
    ctx.font = 'bold 16px monospace'
    
    // Transactions
    ctx.fillStyle = '#222222'
    const txText = `${totalSales}`
    ctx.fillText(txText, 40, 118)
    const txWidth = ctx.measureText(txText).width
    ctx.fillStyle = '#888888'
    ctx.font = '11px monospace'
    ctx.fillText('TRANSACTIONS', 40 + txWidth + 6, 118)
    
    // Volume
    ctx.font = 'bold 16px monospace'
    ctx.fillStyle = '#ff9900'
    const volText = `${totalVolume.toFixed(2)}`
    ctx.fillText(volText, 175, 118)
    const volWidth = ctx.measureText(volText).width
    ctx.fillStyle = '#888888'
    ctx.font = '11px monospace'
    ctx.fillText('SOL', 175 + volWidth + 4, 118)
    
    // Avg Price
    ctx.font = 'bold 16px monospace'
    ctx.fillStyle = '#222222'
    const avgText = `${avgPrice.toFixed(3)}`
    ctx.fillText(avgText, 290, 118)
    const avgWidth = ctx.measureText(avgText).width
    ctx.fillStyle = '#888888'
    ctx.font = '11px monospace'
    ctx.fillText('AVG', 290 + avgWidth + 4, 118)
    ...
    if (highestSale && highestSale.price > 0) {
      ...
      ctx.textAlign = 'right'
      ctx.fillText('TOP TRANSACTION', hsX - 10, hsY + 15)
      
      ctx.fillStyle = '#ff9900'
      ctx.font = 'bold 18px monospace'
      ctx.fillText(`${highestSale.price.toFixed(3)} SOL`, hsX - 10, hsY + 35)
      
      if (hsNumber) {
        ctx.fillStyle = '#222222'
        ctx.font = 'bold 11px monospace'
        ctx.fillText(`ZENJAKU #${hsNumber}`, hsX - 10, hsY + 52)
      }
      ...
      if (highestSale.buyer) {
        ...
        ctx.fillText(`BUYER: ${shortBuyer}`, hsX - 10, hsY + 68)
      }
    ...
    // Primary Actors section (like leaderboard)
    ctx.fillStyle = '#666666'
    ctx.font = '12px monospace'
    ctx.fillText('PRIMARY ACTORS', 40, 155)

    let yPos = 180
    const imgSize = 44
    const maxImages = 12
    const entryHeight = 105
    ...
    for (let i = 0; i < topWallets.length; i++) {
      ...
      ctx.fillText(`#${i + 1}`, 40, yPos + 20)
      
      // Address (prominent)
      ctx.fillStyle = '#222222'
      ctx.font = 'bold 15px monospace'
      ctx.fillText(shortAddr, 90, yPos + 20)

      // Volume
      ctx.fillStyle = '#ff9900'
      ctx.font = 'bold 15px monospace'
      const volText = data.volume.toFixed(3)
      ctx.fillText(volText, 200, yPos + 20)
      const volWidth = ctx.measureText(volText).width
      ctx.fillStyle = '#888888'
      ctx.font = '10px monospace'
      ctx.fillText('SOL', 200 + volWidth + 3, yPos + 20)

      // Donated & Burned
      ctx.fillText(`DON: ${data.donated.toFixed(5)}`, 200 + volWidth + 45, yPos + 20)
      ctx.fillText(`BRN: ${data.burned.toFixed(5)}`, 200 + volWidth + 160, yPos + 20)
      
      // Full address (subtle, below)
      ctx.fillStyle = '#aaaaaa'
      ctx.font = '9px monospace'
      ctx.fillText(address, 90, yPos + 35)
      ...
      if (items.length === 0) {
        ...
        ctx.font = '10px monospace'
        ctx.fillText('— no image data —', 90, yPos + 60)
      }
      ...
      for (const item of items) {
        try {
          ...
          // Draw image
          ctx.drawImage(img, imgX, yPos + 45, imgSize, imgSize)
          
          // Very subtle color tint on bottom edge only
          ctx.globalAlpha = 0.4
          const gradient = ctx.createLinearGradient(imgX, yPos + 45, imgX, yPos + 45 + imgSize)
          gradient.addColorStop(0, 'transparent')
          gradient.addColorStop(0.7, 'transparent')
          gradient.addColorStop(1, item.type === 'buy' ? '#00cc00' : '#ff3333')
          ctx.fillStyle = gradient
          ctx.fillRect(imgX, yPos + 45, imgSize, imgSize)
          ctx.globalAlpha = 1.0
          
          // Subtle border
          ctx.strokeStyle = item.type === 'buy' ? '#00aa00' : '#cc0000'
          ctx.lineWidth = 1
          ctx.strokeRect(imgX, yPos + 45, imgSize, imgSize)
      ...
      if (data.items.length > maxImages) {
        ctx.fillStyle = '#888888'
        ctx.font = '11px monospace'
        ctx.fillText(`+${data.items.length - maxImages}`, imgX + 4, yPos + 68)
      }
    ...
    if (topWallets.length === 0) {
      ...
      ctx.fillText('No sales in this period', 40, yPos)
    }
    ...
    ctx.fillStyle = '#ff9900'
    ctx.font = '12px monospace'
    ctx.fillText(`DONATED: ${totalDonated.toFixed(5)} SOL · BURNED: ${totalBurned.toFixed(5)} SOL`, 40, h - 70)

    // Footer
    ctx.fillStyle = '#aaaaaa'
    ctx.font = '10px monospace'
    const timestamp = new Date().toISOString()
    ctx.fillText(`Generated: ${timestamp}`, 40, h - 40)
    ctx.fillText('zenjaku.fun/collect', w - 150, h - 40)
