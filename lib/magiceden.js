...
export function processActivitiesForDB(activities, symbol) {
  // Debug: log first activity to see all available fields
  if (activities.length > 0) {
    console.log('[MagicEden] First activity keys:', Object.keys(activities[0]))
    console.log('[MagicEden] First activity sample:', JSON.stringify(activities[0], null, 2))
  }
  ...
  return activities
    ...
    .map(activity => {
        // Magic Eden uses various field names for the token mint address
        // Check all possible locations
        const tokenMint = activity.tokenMint 
          || activity.mint 
          || activity.token_mint 
          || activity.tokenAddress
          || activity.nftAddress
          || (activity.token && activity.token.mintAddress)
          || (activity.token && activity.token.mint)
          || null
        
        if (!tokenMint) {
          console.log(`[MagicEden] WARNING: No tokenMint found for sale. Activity keys:`, Object.keys(activity))
        }
        ...
        return {
            ...
            source: 'magiceden',
            token_mint: tokenMint ? sanitizeString(tokenMint) : null
        };
    })
}
